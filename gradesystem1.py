...
file_handler = logging.FileHandler('gradesystem1_logfile.log')
#file_handler.setLevel(logging.ERROR)
file_handler.setFormatter(formatter)

logger.addHandler(file_handler)

logging.debug('This is a debug message') #Debug message used for diagnosing problems
logging.info('This is a info message') #Info message used for confirmation that things are working as expected
logging.warning('This is a warning message') #Warning message used as an indication of an unexpected event/problem
logging.error('This is a error message') #Error message used for more serious problems where functions are unabled to be performed
logging.critical('This is a critical message')  #Critical message used for a serious error where program may be unable to run
...
def main():
	...
	for row in csv_inputfile:					 		 #iterate over the csv file object by looping over every row, using a for loop
		...
		for i in range(2,totcol):
			...
			logger.info('Values: {} '.format(row[i]))


		avrg = calcavrg(total_subject_percentage,num_of_subjects)
		logger.debug('Sum of values: {} '.format(total_subject_percentage))
		logger.debug('Average: (({}) / {}) = {}'.format(total_subject_percentage, num_of_subjects, avrg))

		grade = determineGrade(avrg);
		logger.debug('Calculated grade symbol for: {} = {}'.format(avrg, grade))

		data = [firstname, surname, avrg, grade];
		logger.info('Writing data to output csv file: {},{},{},{}'.format(firstname, surname, avrg, grade))
...
def determineGrade(average):
	if( 80 <= average <= 100): 	
		...
		average = '-'
		return average;
		#logger.error('Input value out of bounds which caused an undesired average and no symbol to be created, check input values')
